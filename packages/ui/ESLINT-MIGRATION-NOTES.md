...
| `react-hooks/set-state-in-effect`         | `src/contexts/NativeIdentityApiContext.tsx`              | `nativeIdentityClient`, `nativeIdentityApi`, `nativeIdentityApiDescriptor` → `useMemo` (all synchronously derivable from `selectedNetworkInfo`); separate `wsProvider` memo folded in; 3 `useState`s + init `useEffect` removed; remaining effects only set state inside `.then()` callbacks — not flagged                                                                                      |
...
| ✅ `src/contexts/NativeIdentityApiContext.tsx` | 89    | Was miscategorised as B — setters were synchronous; `nativeIdentityClient/Api/ApiDescriptor` → `useMemo`; `wsProvider` memo folded in; 3 `useState`s + init `useEffect` removed              |
| ✅ `src/contexts/PeopleChainApiContext.tsx`    | 67    | Same fix as `NativeIdentityApiContext` — `pplClient/Api/ApiDescriptor` → `useMemo`; `wsProvider` memo folded in; 3 `useState`s + init `useEffect` removed                                   |
...
| File                                 | Line | Notes                                                                                                                                                                                                                                                                                                          |
| ------------------------------------ | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ `src/contexts/NetworkContext.tsx` | 53   | `selectedNetwork` and `selectedNetworkInfo` moved to lazy `useState` initialisers (reading `searchParams` and `localStorage` synchronously); interface narrowed to non-optional; a mount-only `useEffect` (empty deps + `eslint-disable exhaustive-deps`) handles the one-time URL and localStorage write-back |
